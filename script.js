...
class Balloon {
    ...
    draw() {
        ...
        // Add more colors for higher health values
        if (this.health > 3) {      // Example: Yellow for 4 health
             ctx.fillStyle = 'yellow';
        } else if (this.health > 2) { // Green for 3 health
            ctx.fillStyle = 'green';
        } else if (this.health > 1) { // Blue for 2 health
            ctx.fillStyle = 'blue';
        } else {                      // Red for 1 health
            ctx.fillStyle = 'red';
        }
    ...
    takeDamage(amount) {
        ...
        if (this.health <= 0) {
            ...
            updateUI();
             // Note: Color change happens on next draw cycle based on reduced health
             // If you wanted instant visual change (e.g., blue turning red),
             // you'd add more logic here, potentially replacing the balloon object.
             // For simplicity, we let the next draw handle the new color if health > 0.
        }
    }
}
...
function spawnBalloon() {
    ...
    if (waveSpawnIndex < waveBalloonTypes.length && spawnCounter <= 0) {
        ...
        balloonsToSpawn--;     // Decrement remaining count (used for wave end check)
